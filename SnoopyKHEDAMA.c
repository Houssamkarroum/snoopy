...
void loadGame();
void controls(void);
void updateBallPosition(int * ballX, int * ballY, int maxX, int maxY, int * ballDirectionX, int * ballDirectionY);
...
void gestionMouvement(char niveau[HAUTEUR_NIVEAU][LARGEUR_NIVEAU], int * posX, int * posY, int * ballX, int * ballY, int commande) {
  ...
  case UP_ARROW:
    if ( * posY > 0 && niveau[ * posY - 1][ * posX] != '#') {
      ( * posY) --;
    }
    break;
  case DOWN_ARROW:
    if ( * posY < HAUTEUR_NIVEAU - 1 && niveau[ * posY + 1][ * posX] != '#') {
      ( * posY) ++;
    }
    break;
  case LEFT_ARROW:

    if ( * posX > 0 && niveau[ * posY][ * posX - 1] != '#') {
      ( * posX) --;
    }
    break;
  case RIGHT_ARROW:
    if ( * posX < LARGEUR_NIVEAU - 1 && niveau[ * posY][ * posX + 1] != '#') {
      ( * posX) ++;
    }
    break;
  case EXIT_BUTTON:
...
void loadGame() {
  ...
  while (1) {
    ...
    updateBallPosition( & ballX, & ballY, LARGEUR_NIVEAU, HAUTEUR_NIVEAU, & ballDirectionX, & ballDirectionY);
